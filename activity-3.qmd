...
results_by_n |>
  ...
  dplyr::distinct(n, value, measure) |>
...
```{webr}
results_by_n |>
  tidyr::pivot_longer(cols = c(power, coverage)) |>
...
or_prenatal_for_rd <- function(target_rd) {
  # the below takes a target risk difference and finds the log-odds ratio that would produce it, given the other parameters of the DGM
...
::: {.callout-tip appearance="minimal"}
The local (downloads) version uses N = 10,000, with 250 replicates at each of 6 hypothesized risk differences. Here in the browser we use N = 4,000, with 100 replicates at each.
:::
...
### Plot: power vs. hypothesized effect size, at N = 4,000
